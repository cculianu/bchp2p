...
bool BCLog::Logger::OpenDebugLog() {
    ...
    m_fileout = std::fopen(m_file_path.c_str(), "a");
    if (!m_fileout) {
        return false;
    }
...
void BCLog::Logger::LogPrintStr(std::string &&str)
{
    ...
    if (m_print_to_file) {
        ...
        if (m_fileout == nullptr) {
            ...
            if (m_reopen_file) {
                m_reopen_file = false;
                FILE *new_fileout = std::fopen(m_file_path.c_str(), "a");
                if (new_fileout) {
                    // unbuffered.
                    setbuf(m_fileout, nullptr);
                    fclose(m_fileout);
...
void BCLog::Logger::ShrinkDebugFile() {
    ...
    FILE *file = std::fopen(m_file_path.c_str(), "r");

    // Special files (e.g. device nodes) may not have a size.
    size_t log_size = 0;
    try {
        log_size = std::filesystem::file_size(m_file_path);
    ...
    if (file && log_size > 11 * (RECENT_DEBUG_HISTORY_SIZE / 10)) {
        ...
        int nBytes = std::fread(vch.data(), 1, vch.size(), file);
        std::fclose(file);

        file = std::fopen(m_file_path.c_str(), "w");
        if (file) {

...
Amount CFeeRate::GetFee(size_t nBytes) const {
    return detail::GetFee<false>(nBytes, nSatoshisPerK);
}

Amount CFeeRate::GetFeeCeiling(size_t nBytes) const {
    return detail::GetFee<true>(nBytes, nSatoshisPerK);
}

std::string CFeeRate::ToString() const {
    return strprintf("%d.%08d %s/kB", nSatoshisPerK / COIN, (nSatoshisPerK % COIN) / SATOSHI, GetCurrencyUnit());
}
